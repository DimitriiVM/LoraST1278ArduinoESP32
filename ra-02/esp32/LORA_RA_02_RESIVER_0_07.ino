...
char TYPEDEV[12] = "";
char DATA[12] = "";
char Message[128]; //Входяшие  данные по USB
char DataLora[128]; //Входящие  данные  для LORA
...
void USB()
{
  ...
  {
    int Buf = Serial.readBytes(Message, 128);
    ...
    else if(strncmp(Message, "sf=", 3) == 0)
    {
      char CharTemp[3] = {0x00, 0x00};
      for(int i = 0; i < 2; i++)
      {
        if(Message[3 + i] <= 0x39 && Message[3 + i] >= 0x30)
        {
          CharTemp[i] = Message[3 + i];
        }
        else
        {
          break;
      ...
      SpreadingFactor = temp;
      Serial.printf("SpreadingFactor:%d", SpreadingFactor);
      EEPROM.write(3, SpreadingFactor);
      EEPROM.commit();
      LoRa.setSpreadingFactor(SpreadingFactor);
    }
    ClearMessage();//От  греха по дальше чистим  кэш. чтобы  поднять стабильность работы
  }
}
...
void Configuration()
{
  ...
  Serial.printf("POWER LORA:%d\r\n", txPower);
  SpreadingFactor = EEPROM.read(3); //Коэф. SF
